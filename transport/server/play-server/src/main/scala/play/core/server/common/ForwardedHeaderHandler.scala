...
/**
 * The ForwardedHeaderHandler class works out the remote address and protocol by taking into account Forward
 * and X-Forwarded-* headers from trusted proxies. The algorithm it uses is as follows:
 *
 *   1. Start with the immediate connection to the application.
 *   2. If the proxy *did not* send a valid Forward or X-Forward-* header then return that connection and
 *      don't do any further processing.
 *   3. If the proxy *did* send a valid header then work out whether we trust it by checking whether the
 *      immediate connection is in our list of trusted proxies.
 *   4. If the immediate connection *is* a trusted proxy, then resume at step 1 using the connection info in
 *      the forwarded header.
 *   5. If the immediate connection *is not* a trusted proxy, then return the immediate connection info and
 *      don't do any further processing.
 *
 * Each address is associated with a secure or insecure protocol by pairing it with a `proto` entry in the
 * headers. If the `proto` entry is missing or if `proto` entries can't be matched with addresses, then the
 * default is insecure.
 *
 * It is configured by two configuration options: <dl> <dt>play.http.forwarded.version</dt> <dd> The version
 * of the forwarded headers ist uses to parse the headers. It can be <code>x-forwarded</code> for legacy
 * headers or <code>rfc7239</code> for the definition from the RFC 7239 <br> Default is x-forwarded. </dd>
 * <dt>play.http.forwarded.trustedProxies</dt> <dd> A list of proxies that are ignored when getting the remote
 * address or the remote port. It can have optionally an address block size. When the address block size is
 * set, all IP-addresses in the range of the subnet will be treated as trusted. </dd> </dl>
 */
private[server] class ForwardedHeaderHandler(configuration: ForwardedHeaderHandlerConfig):
   ...
   *
   * @param rawConnection
   *   The raw connection that connected to the Play server.
   * @param headers
   *   The request headers.
   * @return
   *   An updated connection
   */
  def forwardedConnection(rawConnection: RemoteConnection, headers: Headers): RemoteConnection =
...
private[server] object ForwardedHeaderHandler:
  ...
  /**
   * An unparsed address and protocol pair from a forwarded header. Both values are optional.
   */
  ...
  case class ForwardedHeaderHandlerConfig(version: ForwardedHeaderVersion, trustedProxies: List[Subnet]):
    ...
    /**
     * Removes surrounding quotes if present, otherwise returns original string. Not RFC compliant. To be
     * compliant we need proper header field parsing.
     */
    ...
    /**
     * Parse any Forward or X-Forwarded-* headers into a sequence of ForwardedEntry objects. Each object a
     * pair with an optional unparsed address and an optional unparsed protocol. Further parsing may happen
     * later, see `remoteConnection`.
     */
    def forwardedHeaders(headers: Headers): Seq[ForwardedEntry] = version match
      ...
      case Xforwarded =>
        ...
        if forHeaders.length == protoHeaders.length then
          forHeaders.zip(protoHeaders).map { case (f, p) =>
          ...
          // default.
          forHeaders.map { case f =>
            ForwardedEntry(Some(f), None)
          }

    /**
     * Try to parse a `ForwardedEntry` into a valid `ConnectionInfo` with an IP address and information about
     * the protocol security. If this cannot happen, either because parsing fails or because the connection
     * info doesn't include an IP address, this method will return `Left` with an error message.
     */
    ...
    /**
     * Check if a connection is considered to be a trusted proxy, i.e. a proxy whose forwarding headers we
     * will process.
     */
