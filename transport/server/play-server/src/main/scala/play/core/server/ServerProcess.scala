...
/**
 * Abstracts a JVM process so it can be mocked for testing or to isolate pseudo-processes within a VM. Code
 * using this class should use the methods in this class instead of methods like `System.getProperties()`,
 * `System.exit()`, etc.
 */
trait ServerProcess:
...
/**
 * A ServerProcess that wraps a real JVM process. Calls have a real effect on the JVM, e.g. `exit` calls
 * `System.exit.`
 */
