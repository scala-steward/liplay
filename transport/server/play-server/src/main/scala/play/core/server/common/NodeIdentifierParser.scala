 ...
 *
 * @param version
 *   The version of the forwarded headers that we want to parse nodes for. The version is used to switch
 *   between IP address parsing behavior.
 */
private[common] class NodeIdentifierParser(version: ForwardedHeaderVersion) extends RegexParsers:
  ...
  private lazy val node = phrase(nodename ~ opt(":" ~> nodeport)) ^^ { case x ~ y =>
    x -> y
  }
