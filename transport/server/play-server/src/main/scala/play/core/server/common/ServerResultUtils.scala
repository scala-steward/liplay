...
private[play] final class ServerResultUtils(
  ...
  /**
   * Determine whether the connection should be closed, and what header, if any, should be added to the
   * response.
   */
  def determineConnectionHeader(request: RequestHeader, result: Result): ConnectionHeader =
    ...
    else if result.header.headers.get(CONNECTION).exists(_.equalsIgnoreCase(CLOSE)) then DefaultClose
    else if (result.body.isInstanceOf[HttpEntity.Streamed] && result.body.contentLength.isEmpty) ||
   ...
   */
  def validateResult(request: RequestHeader, result: Result, httpErrorHandler: HttpErrorHandler)(using
      mat: Materializer
  ): Future[Result] =
    if request.version == HttpProtocol.HTTP_1_0 && result.body.isInstanceOf[HttpEntity.Chunked] then
      cancelEntity(result.body)
      val exception =
        new ServerResultException("HTTP 1.0 client does not support chunked response", result, null)
   ...
   *   2. If there's an error, calls the `HttpErrorHandler` to get a new `Result`, then converts that.
   *   3. If there's an error with *that* `Result`, uses the `DefaultHttpErrorHandler` to get another
   *      `Result`, then converts that.
   *   4. Hopefully there are no more errors. :)
   *   5. If calling an `HttpErrorHandler` throws an exception, then a fallback response is returned, without
   *      an conversion.
   */
  def resultConversionWithErrorHandling[R](
    ...
    def handleConversionError(conversionError: Throwable): Future[R] =
      ...
      try
        ...
        futureErrorResult.flatMap { errorResult =>
          ...
          resultConversionWithErrorHandling(requestHeader, errorResult, DefaultHttpErrorHandler)(
            resultConverter
          )(
            fallbackResponse
          )
    ...
    catch case NonFatal(e) => handleConversionError(e)
  ...
  /**
   * No `Connection` header should be sent. Used on an HTTP 1.0 connection where the default behavior is to
   * close the connection, or when the response already has a Connection: close header.
   */
  ...
  /**
   * No `Connection` header should be sent. Used on an HTTP 1.1 connection where the default behavior is to
  ...
  /**
   * Given a map of headers, split it into a sequence of individual headers. Most headers map into a single
   * pair in the new sequence. The exception is the `Set-Cookie` header which we split into a pair for each
   * cookie it contains. This allows us to work around issues with clients that can't handle combined headers.
   * (Also RFC6265 says multiple headers shouldn't be folded together, which Play's API unfortunately does.)
   */
