...
class NettyServer(
  ...
  private val maxContentLength =
    Server.getPossiblyInfiniteBytes(serverConfig.underlying, "max-content-length")
  ...
  private def setOptions(
    ...
    config.entrySet().asScala.filterNot(_.getKey.startsWith("child.")).foreach { option =>
      ...
      if ChannelOption.exists(cleanKey) then
        logger.debug(s"Setting Netty channel option ${cleanKey} to ${unwrap(option.getValue)}${
            if bootstrapping then " at bootstrapping"
            else ""
          }")
  ...
  private def channelSink(port: Int): Sink[Channel, Future[Done]] =
    Sink.foreach[Channel] { (connChannel: Channel) =>
      ...
      if logWire then pipeline.addLast("logging", new LoggingHandler(LogLevel.DEBUG))
      ...
      val requestHandler =
        new PlayRequestHandler(this, maxContentLength, application, resultUtils, modelConversion)
  ...
  private def bindChannel(port: Int): Channel =
    ...
    if boundAddress == null then
      ...
      throw e
    if mode != Mode.Test then logger.info(s"Listening for $protocolName on $boundAddress")
    serverChannel
  ...
  private def initializeChannelOptionsStaticMembers(): Unit =
    ...
    Seq(classOf[ChannelOption[?]], classOf[UnixChannelOption[?]], classOf[EpollChannelOption[?]]).foreach(
      clazz =>
 ...
 * }}}
 *
 * Use this together with <a href="https://www.playframework.com/documentation/latest/ScalaSirdRouter">Sird
 * Router</a>.
 */
object NettyServer:
