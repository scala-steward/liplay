...
private[play] class NettyIdleHandler extends ChannelInboundHandlerAdapter:
  ...
  /**
   * Originally, this method lived within [[PlayRequestHandler]]. However, the [[PlayRequestHandler]] gets
   * removed from the Netty pipeline when a http connection gets upgraded to a websocket connection.
   * Therefore, for websockets, a user events would not be handled anymore, not calling this method and
