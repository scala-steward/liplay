...
object RoutesCompiler:
  ...
  object GeneratedSource:
    def unapply(file: File): Option[GeneratedSource] =
      val lines: Array[String] = if file.exists then
        try Files.readAllLines(file.toPath, Charset.forName(implicitly[Codec].name)).asScala.toArray[String]
        catch
      ...
      if lines.contains("// @GENERATOR:play-routes-compiler") then
        Some(new GeneratedSource:
          val source: Option[File] =
            lines.find(_.startsWith("// @SOURCE:")).map(m => new File(m.trim.drop(11)))

          def mapLine(generatedLine: Int): Option[Int] =
            lines.view.take(generatedLine).reverse.collectFirst { case LineMarker(line) =>
   ...
   *
   * @param file
   *   The routes file to compile.
   * @param additionalImports
   *   The additional imports.
   * @param forwardsRouter
   *   Whether a forwards router should be generated.
   * @param reverseRouter
   ...
   *
   * @param task
   *   The routes compilation task
   * @param generator
   *   The routes generator
   * @param generatedDir
   *   The directory to place the generated source code in
   * @return
   *   Either the list of files that were generated (right) or the routes compilation errors (left)
   */
  def compile(
