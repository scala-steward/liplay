...
object RoutesFileParser:
   ...
   *
   * @param routesFile
   *   The routes file to parse
   * @return
   *   Either the list of compilation errors encountered, or a list of routing rules
   */
   ...
   *
   * @param routesContent
   *   The content of the routes file
   * @param routesFile
   *   The routes file (used for error reporting)
   * @return
   *   Either the list of compilation errors encountered, or a list of routing rules
   */
  ...
  private def validate(file: java.io.File, routes: List[Route]): Seq[RoutesCompilationError] =
    ...
    routes.foreach { route =>
      if route.call.controller.isEmpty then
...
private[routes] class RoutesFileParser extends JavaTokenParsers:
  ...
  def modifiers: Parser[List[Modifier]] =
    "+" ~> ignoreWhiteSpace ~> repsep("""[^#\s]+""".r, separator) <~ ignoreWhiteSpace ^^ (_.map(
      Modifier.apply
    ))
  ...
  def parentheses: Parser[String] =
    "(" ~ (several((parentheses | not(")") ~> """.""".r))) ~ commit(")") ^^ { case p1 ~ charList ~ p2 =>
  ...
  def singleComponentPathPart: Parser[DynamicPart] = (":" ~> identifier) ^^ { case name =>
    DynamicPart(name, """[^/]+""", encode = true)
  }

  def multipleComponentsPathPart: Parser[DynamicPart] = ("*" ~> identifier) ^^ { case name =>
  ...
  def simpleType: Parser[String] =
    ((stableId <~ ignoreWhiteSpace) ~ opt(typeArgs)) ^^ { case sid ~ ta =>
      sid.toString + ta.getOrElse("")
    } |
      (space("(") ~ types ~ space(")")) ^^ { case _ ~ b ~ _ =>
        "(" + b + ")"
      }

  def typeArgs: Parser[String] =
    (space("[") ~ types ~ space("]") ~ opt(typeArgs)) ^^ { case _ ~ ts ~ _ ~ ta =>
      "[" + ts + "]" + ta.getOrElse("")
    } |
      (space("#") ~ identifier ~ opt(typeArgs)) ^^ { case _ ~ id ~ ta =>
        "#" + id + ta.getOrElse("")
      }
  ...
  def parameterFixedValue: Parser[String] = "=" ~ ignoreWhiteSpace ~ expression ^^ { case a ~ _ ~ b =>
    a + b
  }

  def parameterDefaultValue: Parser[String] = "?=" ~ ignoreWhiteSpace ~ expression ^^ { case a ~ _ ~ b =>
    a + b
  }
  ...
  def absoluteMethod: Parser[List[String]] =
    namedError(
      ident ~ "." ~ rep1sep(ident, ".") ^^ { case first ~ _ ~ rest =>
        first :: rest
      },
  ...
  def route = httpVerb ~! separator ~ path ~ separator ~ positioned(call) ^^ { case v ~ _ ~ p ~ _ ~ c =>
    Route(v, p, c)
  }

  def include = "->" ~! separator ~ path ~ separator ~ router ^^ { case _ ~ _ ~ p ~ _ ~ r =>
  ...
  def sentence: Parser[Product] =
    ignoreWhiteSpace ~>
      namedError(
        Seq[Parser[Product]](comment, modifiersWithComment, positioned(include), positioned(route))
          .reduceLeft(_ | _),
        "HTTP Verb (GET, POST, ...), include (->), comment (#), or modifier line (+) expected"
      ) <~ ignoreWhiteSpace <~ (newLine | EOF)

  def parser: Parser[List[Rule]] = phrase((blankLine `|` sentence `*`) <~ end) ^^ { case routes =>
    routes.reverse
