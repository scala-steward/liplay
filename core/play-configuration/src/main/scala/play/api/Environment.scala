...
case class Environment(rootPath: File, classLoader: ClassLoader, mode: Mode):
   ...
   *
   * Note that it is up to you to manage the files in the application root path in production. By default,
   * there will be nothing available in the application root path.
   *
  ...
  /**
   * Retrieves a file relative to the application root path. This method returns an Option[File], using None
   ...
   *
   * The conf directory is included on the classpath, so this may be used to look up resources, relative to
   * the conf directory.
   *
