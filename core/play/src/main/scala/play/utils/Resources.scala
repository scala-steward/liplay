...
object Resources:
   ...
   *
   * Depends on the URL connection type whether it's accurate. If it's unable to determine whether it's a
  ...
  def isUrlConnectionADirectory(urlConnection: URLConnection) = urlConnection match
    ...
    case jar: JarURLConnection =>
      if jar.getJarEntry.isDirectory then true
      else
   ...
   *
   * This works around a JDK bug where if the URL connection is to a JAR file, and the entry is a directory,
   * an NPE is thrown.
   */
  def closeUrlConnection(connection: URLConnection): Unit =
    connection match
      case jar: JarURLConnection =>
        if !jar.getUseCaches then jar.getJarFile.close()
      case other =>
