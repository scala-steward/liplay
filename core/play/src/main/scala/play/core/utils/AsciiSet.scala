...
object AsciiSet:
  ...
  def apply(c: Char, cs: Char*): AsciiSet = cs.foldLeft[AsciiSet](apply(c)) { case (acc, c1) =>
    acc ||| apply(c1)
  }
...
/**
 * A set of ASCII characters. The set should be built out of [[AsciiRange]], [[AsciiChar]], [[AsciiUnion]],
 * etc then converted to an [[AsciiBitSet]] using `toBitSet` for fast querying.
 */
trait AsciiSet:

  /**
   * The internal method used to query for set membership. Doesn't do any bounds checks. Also may be slow, so
   * to query from outside this package you should convert to an [[AsciiBitSet]] using `toBitSet`.
   */
  ...
  def toBitSet: AsciiBitSet =
    ...
    for i <- 0 until 256 do if this.getInternal(i) then bitSet.set(i)
...
/**
 * An efficient representation of a set of ASCII characters. Created by building an [[AsciiSet]] then calling
 * `toBitSet` on it.
 */
