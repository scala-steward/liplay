...
object Lang:
   ...
   *
   * Pre 2.6.x, defaultLang was an implicit value, meaning that it could be used in implicit scope resolution
   * if no Lang was found in local scope. This setting was too general and resulted in bugs where the
   * defaultLang was being used instead of a request.lang, if request was not declared as implicit.
   */
