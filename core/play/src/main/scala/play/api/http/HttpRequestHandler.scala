...
trait HttpRequestHandler:
   ...
   *
   * In addition to retrieving a handler for the request, the request itself may be modified - typically it
...
/**
 * A base implementation of the [[HttpRequestHandler]] that handles Scala actions. If you use Java actions in
 * your application, you should override [[JavaCompatibleHttpRequestHandler]]; otherwise you can override this
 * for your custom handler.
 *
 * Technically, this is not the default request handler that Play uses, rather, the
...
class DefaultHttpRequestHandler(
  ...
  override def handlerForRequest(request: RequestHeader): (RequestHeader, Handler) =
    ...
    /**
     * Call the router to get the handler, but with a couple of types of fallback. First, if a HEAD request
   ...
   *
   * This method can be overridden if you want to provide some custom routing strategies, for example, using
   * different routers based on various request parameters.
   *
   * @param request
   *   The request
   * @return
