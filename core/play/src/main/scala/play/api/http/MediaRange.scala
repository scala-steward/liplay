  ...
  override def toString =
    mediaType + "/" + mediaSubType + parameters
      .map { param =>
        "; " + param._1 + param._2
          .map { value =>
            if MediaRangeParser.token(new CharSequenceReader(value)).next.atEnd then "=" + value
            else "=\"" + value.replaceAll("\\\\", "\\\\\\\\").replaceAll("\"", "\\\\\"") + "\""
          }
          .getOrElse("")
      }
      .mkString("")
    ...
    def apply(mediaType: String): Option[MediaType] =
      MediaRangeParser.mediaType(new CharSequenceReader(mediaType)) match
        case MediaRangeParser.Success(mt: MediaType, next) =>
          if !next.atEnd then logger.debug(s"Unable to parse part of media type '${next.source}'")
          Some(mt)
...
object MediaRange:
  ...
  /**
   * Given a list of acceptable media ranges, find the preferred media type in the list of available media
   * types.
   *
   * Note: the media types in the list should be without parameters, e.g. `text/html` not
    ...
    def apply(mediaRanges: String): Seq[MediaRange] =
      MediaRangeParser(new CharSequenceReader(mediaRanges)) match
        case MediaRangeParser.Success(mrs: List[MediaRange], next) =>
          if !next.atEnd then logger.debug(s"Unable to parse part of media range header '${next.source}'")
          mrs.sorted
   ...
   *
   * The reason it is highest to lowest instead of lowest to highest is to ensure sorting is stable, so if two
   * media ranges have the same ordering weight, they will not change order.
   *
   * Ordering rules for MediaRanges:
   *
   * First compare by qValue, default to 1. Higher means higher priority. Then compare the media type. If they
   * are not the same, then the least specific (ie, if one is *) has a lower priority, otherwise if they have
   * same priority. Then compare the sub media type. If they are the same, the one with the more parameters
   * has a higher priority. Otherwise the least specific has the lower priority, otherwise they have the same
   * priority.
   */
  ...
  private[http] object MediaRangeParser extends Parsers:
     ...
     */
    val parameter = token ~ opt('=' ~> (token | quotedString)) <~ rep(' ') ^^ { case name ~ value =>
      name -> value
    }
