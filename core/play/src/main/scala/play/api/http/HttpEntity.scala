 ...
 * An HTTP entity.
 *
 * HTTP entities come in three flavors, [[HttpEntity.Strict]], [[HttpEntity.Streamed]] and
 * [[HttpEntity.Chunked]].
 */
sealed trait HttpEntity:
   ...
   *
   * If this returns true, then the entity is definitely empty. If it returns false, the entity may or may not
   * be empty.
   */
...
object HttpEntity:
   ...
   *
   * @param chunks
   *   The stream of chunks for this entity. Must be zero or more [[HttpChunk.Chunk]] elements, followed by
   *   zero or one [[HttpChunk.LastChunk]] elements. Any elements after the [[HttpChunk.LastChunk]] element
   *   will be ignored. If no [[HttpChunk.LastChunk]] element is sent, then the last chunk will contain no
   *   trailers.
   * @param contentType
   *   The content type, if known.
   */
 ...
 * An HTTP chunk.
 *
 * May either be a [[HttpChunk.Chunk]] containing data, or a [[HttpChunk.LastChunk]], signifying the last
 * chunk in a stream, optionally with trailing headers.
 */
