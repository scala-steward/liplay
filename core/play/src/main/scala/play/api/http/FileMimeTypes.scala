 ...
 *
 * This trait is primarily used with results and assets that send files, for users who want to send a file
 * without having to specify an explicit content type. For example, a user can send a file with ".json":
 *
 * {{{
 ...
 * }}}
 *
 * and have a "json" -> "application/json" mapping done implicitly based off the file extension. The Assets
 ...
 * }}}
 *
 * or, if [[play.api.mvc.BaseController]] is extended, then an implicit fileMimeTypes instance is already made
