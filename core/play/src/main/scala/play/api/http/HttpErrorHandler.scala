...
/**
 * An [[HttpErrorHandler]] that delegates to one of several [[HttpErrorHandler]]s based on media type
 * preferences.
 *
 * For example, to create an error handler that handles JSON and HTML, with JSON preferred by the app as
 * default:
 * {{{
 ...
 * }}}
 *
 * If the client's preferred media range matches multiple media types in the list, then the first match is
 * chosen.
 */
 ...
 *
 * In Dev mode, exceptions thrown by the server code will be rendered in JSON messages. In Prod mode, they
 * will not be rendered.
 *
 * You could override how exceptions are rendered in Dev mode by extending this class and overriding the
...
class JsonHttpErrorHandler(environment: Environment) extends HttpErrorHandler:
   ...
   *
   * <p>As a last resort this method allows you to return a (simple) error message that will be send along
   * with a "500 Internal Server Error" response. It's highly recommended to just return a simple JsonNode,
   * without doing any fancy processing inside the method (like accessing files,...) that could throw
   * exceptions. This is your last chance to send a meaningful error message when everything else failed.
   *
   * @param request
   ...
   *   The server error.
   * @return
   *   An error JSON which will be send as last resort in case handling a server error with this error handler
   *   failed.
   */
