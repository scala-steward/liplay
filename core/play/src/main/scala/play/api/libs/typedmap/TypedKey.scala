 ...
 * This class uses reference equality for comparisons, so each new instance is different key.
 *
 * @param displayName
 *   The name to display for this key or `null` if no display name has been provided. This name is only used
 *   for debugging. Keys with the same name are not considered to be equal.
 * @tparam A
