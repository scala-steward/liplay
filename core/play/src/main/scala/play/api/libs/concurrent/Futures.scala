 ...
 *
 * You can dependency inject the Futures as follows to create a Future that will timeout after a certain
 * period of time:
 *
 * {{{
 ...
 * }}}
 *
 * You should check for timeout by using `scala.concurrent.Future.recover` or
 * `scala.concurrent.Future.recoverWith` and checking for [[scala.concurrent.TimeoutException]]:
 *
 * {{{
 ...
 *
 * You can dependency inject the ActorSystem as follows to create a Future that will timeout after a certain
 * period of time:
 *
 * {{{
...
trait LowPriorityFuturesImplicits:
  implicit class FutureOps[T](future: Future[T]):
    ...
    /**
     * Creates a future which will resolve to a timeout exception if the given [[scala.concurrent.Future]] has
     * not successfully completed within timeoutDuration.
     *
     * Note that timeout is not the same as cancellation. Even in case of timeout, the given future will still
     * complete, even though that completed value is not returned.
     *
