...
trait CSRFTokenSigner:
   ...
   *
   * This primarily exists to defeat the BREACH vulnerability, as it allows the token to effectively be random
   * per request, without actually changing the value.
   *
   * @param token
   *   The token to sign
   * @return
   ...
   *
   * Given a length that both Strings are equal to, this method will always run in constant time. This
   * prevents timing attacks.
   *
   * @deprecated
   *   Please use `java.security.MessageDigest.isEqual(a.getBytes("utf-8"), b.getBytes("utf-8"))` over this
   *   method.
   */
  @deprecated(
...
object CSRFTokenSigner:
  ...
  /**
   * @deprecated
   *   Please use [[java.security.MessageDigest.isEqual]] over this method.
   */
