...
trait Constraints:
  ...
  def emailAddress(errorMessage: String = "error.email"): Constraint[String] =
    Constraint[String]("constraint.email") { e =>
      if e == null then Invalid(ValidationError(errorMessage))
      else if e.trim.isEmpty then Invalid(ValidationError(errorMessage))
      else
        emailRegex
          .findFirstMatchIn(e)
          .map(_ => Valid)
  ...
  def nonEmpty(errorMessage: String = "error.required"): Constraint[String] =
    Constraint[String]("constraint.required") { o =>
      if o == null then Invalid(ValidationError(errorMessage))
      else if o.trim.isEmpty then Invalid(ValidationError(errorMessage))
      else Valid
    }
  ...
  /**
   * Defines a minimum value for `Ordered` values, by default the value must be greater than or equal to the
  ...
  /**
   * Defines a maximum value for `Ordered` values, by default the value must be less than or equal to the
   * constraint parameter
   *
  ...
  /**
   * Defines a minimum length constraint for `String` values, i.e. the string’s length must be greater than or
   * equal to the constraint parameter
   *
  ...
  def minLength(length: Int, errorMessage: String = "error.minLength"): Constraint[String] =
    Constraint[String]("constraint.minLength", length) { o =>
      ...
      if o == null then Invalid(ValidationError(errorMessage, length))
      else if o.size >= length then Valid
      else Invalid(ValidationError(errorMessage, length))
    }

  /**
   * Defines a maximum length constraint for `String` values, i.e. the string’s length must be less than or
   * equal to the constraint parameter
   *
  ...
  /**
   * Defines a regular expression constraint for `String` values, i.e. the string must match the regular
   * expression pattern
   *
   * '''name'''[constraint.pattern(regex)] or defined by the name parameter. '''error'''[error.pattern(regex)]
   * or defined by the error parameter.
   */
  def pattern(
    ...
    if o == null then Invalid(ValidationError(error, regex))
...
object ValidationError:
  ...
  def fromJsonValidationError(jve: JsonValidationError): ValidationError =
    ValidationError(
      jve.message,
      jve.args.collect { case arg: Matchable =>
        arg
      }*
    )
