...
object Formats:
  ...
  private def numberFormatter[T](convert: String => T, real: Boolean = false): Formatter[T] =
    val (formatString, errorString) =
      if real then ("format.real", "error.real") else ("format.numeric", "error.number")
    new Formatter[T]:
  ...
  def dateFormat(pattern: String, timeZone: TimeZone = TimeZone.getDefault): Formatter[Date] =
    new Formatter[Date]:
      ...
      def unbind(key: String, value: Date) = Map(
        key -> formatter.format(value.toInstant.atZone(javaTimeZone))
      )
  ...
  ): Formatter[java.sql.Timestamp] =
    new Formatter[java.sql.Timestamp]:
      import java.time.LocalDateTime

      private val formatter =
        java.time.format.DateTimeFormatter.ofPattern(pattern).withZone(timeZone.toZoneId)
      private def timestampParse(data: String) =
        java.sql.Timestamp.valueOf(LocalDateTime.parse(data, formatter))
      ...
      override def unbind(key: String, value: java.sql.Timestamp) = Map(
        key -> value.toLocalDateTime.format(formatter)
      )
   ...
   */
  implicit val localDateTimeFormat: Formatter[java.time.LocalDateTime] = localDateTimeFormat(
    "yyyy-MM-dd HH:mm:ss"
  )
