...
trait RequestHeader:
  ...
  /**
   * The complete request URI, containing both path and query string. The URI is what was on the status line
   * after the request method. E.g. in "GET /foo/bar?q=s HTTP/1.1" the URI should be /foo/bar?q=s. It could be
   * absolute, some clients send absolute URLs, especially proxies, e.g. http://www.example.org/foo/bar?q=s.
   *
   ...
   *
   * @return
   *   The query parameter's value if the parameter is present and there is only one value. If the parameter
   *   is absent or there is more than one value for that parameter then `None` is returned.
   */
  ...
  /**
   * The HTTP host (domain, optionally port). This value is derived from the request target, if a hostname is
   * present. If the target doesn't have a host then the `Host` header is used, if present. If that's not
   * present then an empty string is returned.
   */
   ...
   * [[play.api.mvc.request.RequestAttrKey.Cookies]]. The attribute uses a Cell to store the cookies, to allow
   * them to be evaluated on-demand.
   */
  def cookies: Cookies = attrs(RequestAttrKey.Cookies).value

  /**
   * Parses the `Session` cookie and returns the `Session` data. The request's session cookie is stored in an
   * attribute indexed by [[play.api.mvc.request.RequestAttrKey.Session]]. The attribute uses a
   * [[play.api.mvc.request.Cell]] to store the session cookie, to allow it to be evaluated on-demand.
   */
  def session: Session = attrs(RequestAttrKey.Session).value

  /**
   * Parses the `Flash` cookie and returns the `Flash` data. The request's flash cookie is stored in an
   * attribute indexed by [[play.api.mvc.request.RequestAttrKey.Flash]]. The attribute uses a
   * [[play.api.mvc.request.Cell]] to store the flash, to allow it to be evaluated on-demand.
   */
  ...
  /**
   * The media type of this request. Same as contentType, except returns a fully parsed media type with
   * parameters.
   */
...
object RequestHeader:
  ...
  /**
   * @return
   *   The items of an Accept* header, with their q-value.
   */
