 ...
 */
case class MultipartFormData[A](
...
object MultipartFormData:
    ...
    def transformRefToBytes(): ByteString =
      refToBytes(ref)
        .orElse(ref match
          ...
          case stf: play.api.libs.Files.TemporaryFile =>
            Some(ByteString.fromArray(Files.readAllBytes(stf.path)))
  ...
  /**
   * The multipart/form-data parser buffers many things in memory, including data parts, headers, file names
   * etc.
   *
   * Some buffer limits apply to each element, eg, there is a buffer for headers before they are parsed. Other
   * buffer limits apply to all in memory data in aggregate, this includes data parts, file names, part names.
   *
...
case class RawBuffer(
  ...
  @volatile private var outStream: OutputStream = scala.compiletime.uninitialized

  private[play] def push(chunk: ByteString): Unit =
    if inMemory != null then
      if chunk.length + inMemory.size > memoryThreshold then
        backToTemporaryFile()
        outStream.write(chunk.toArray)
      else inMemory = inMemory ++ chunk
    else outStream.write(chunk.toArray)

  private[play] def close(): Unit = if outStream != null then outStream.close()
   ...
   */
  def size: Long =
    if inMemory != null then inMemory.size else Files.size(backedByTemporaryFile)

  /**
   * Returns the buffer content as a bytes array.
   *
   * This operation will cause the internal collection of byte arrays to be copied into a new byte array on
   * each invocation, no caching is done. If the buffer has been written out to a file, it will read the
   * contents of the file.
   *
   * @param maxLength
   *   The max length allowed to be stored in memory. If this is smaller than memoryThreshold, and the buffer
   *   is already in memory then None will still be returned.
   * @return
  ...
  def asBytes(maxLength: Long = memoryThreshold): Option[ByteString] =
    if size <= maxLength then
      Some(if inMemory != null then inMemory else ByteString(PlayIO.readFile(backedByTemporaryFile.path)))
    else None
...
trait BodyParserUtils:
   ...
   */
  def flatten[A](
      underlying: Future[BodyParser[A]]
  )(using ec: ExecutionContext, mat: Materializer): BodyParser[A] =
  ...
  def when[A](
    ...
    BodyParser(s"conditional, wrapping=$parser") { request =>
      if predicate(request) then parser(request)
      else Accumulator.done(badResult(request).map(Left.apply)(using Execution.trampoline))
    }
   ...
   */
  def maxLength[A](maxLength: Long, parser: BodyParser[A])(using
      mat: Materializer
  ): BodyParser[Either[MaxSizeExceeded, A]] =
    BodyParser(s"maxLength=$maxLength, wrapping=$parser") { request =>
      if BodyParserUtils.contentLengthHeaderExceedsMaxLength(request, maxLength) then
...
object BodyParserUtils:
   ...
   * @return
   *   true if the request's Content-Length header value is greater than maxLength. false otherwise or if the
   *   request does not have a Content-Length header (or if it can't be parsed).
   */
...
object PlayBodyParsers:
  ...
  /**
   * A helper method for creating PlayBodyParsers. The default values are mainly useful in testing, and
   * default the TemporaryFileCreator and HttpErrorHandler to singleton versions.
   */
  def apply(
  ...
  )(using mat: Materializer): PlayBodyParsers =
...
trait PlayBodyParsers extends BodyParserUtils:
   ...
   *
   * Will attempt to parse content with an explicit charset, but will fallback to UTF-8, ISO-8859-1, and
   * finally US-ASCII if incorrect characters are detected.
   *
   * @param maxLength
  ...
  def tolerantText(maxLength: Long): BodyParser[String] =
    tolerantBodyParser("text", maxLength, "Error decoding text body") { (request, bytes) =>
      ...
      decode(charset)
        .recoverWith { case _: CharacterCodingException =>
          decode(UTF_8)
        }
        .recoverWith { case _: CharacterCodingException =>
          decode(ISO_8859_1)
   ...
   *
   * If the charset is not explicitly declared, then the default "charset" parameter value is US-ASCII, per
   * https://tools.ietf.org/html/rfc6657#section-4. Use tolerantText if more flexible character decoding is
   * desired.
   *
   * @param maxLength
   *   Max length (in bytes) allowed or returns EntityTooLarge HTTP response.
  ...
  def text(maxLength: Long): BodyParser[String] =
    BodyParser("text") { request =>
      if request.contentType.exists(_.equalsIgnoreCase("text/plain")) then
        val bodyParser = tolerantBodyParser("text", maxLength, "Error decoding text body") {
          (request, bytes) =>
            val charset = request.charset.fold(US_ASCII)(Charset.forName)
            import java.nio.charset.CodingErrorAction
   ...
   */
  def raw(
      memoryThreshold: Long = DefaultMaxTextLength,
      maxLength: Long = DefaultMaxDiskLength
  ): BodyParser[RawBuffer] =
   ...
   */
  def tolerantJson[A](using reader: Reads[A]): BodyParser[A] = jsonReads(tolerantJson)
  ...
  /**
   * Parse the body as Json if the Content-Type is text/json or application/json, validating the result with
   * the Json reader.
   *
   * @tparam A
   *   the type to read and validate from the body.
   ...
   *   a Json reader for type A.
   */
  def json[A](using reader: Reads[A]): BodyParser[A] = jsonReads(json)

  /**
   * Parse the body as Json given a BodyParser, validating the result with the Json reader.
   */
  private def jsonReads[A](parser: BodyParser[JsValue])(using reader: Reads[A]): BodyParser[A] =
   ...
   *   Form model
   * @param maxLength
   *   Max length (in bytes) allowed or returns EntityTooLarge HTTP response. If `None`, the default
   *   `play.http.parser.maxMemoryBuffer` configuration value is used.
   * @param onErrors
  ...
  def file(to: File, maxLength: Long): BodyParser[File] = BodyParser(s"file, to=$to") { request =>
    import Execution.Implicits.trampoline
    val bodyAccumulator =
      Accumulator(StreamConverters.fromOutputStream(() => Files.newOutputStream(to.toPath))).map(_ =>
        Right(to)
      )
  ...
  private def requestEntityTooLarge(request: RequestHeader) =
    createBadResult("Request Entity Too Large", REQUEST_ENTITY_TOO_LARGE)(request).map(Left(_))(using
  ...
  def temporaryFile(maxLength: Long): BodyParser[TemporaryFile] = BodyParser("temporaryFile") { request =>
    if BodyParserUtils.contentLengthHeaderExceedsMaxLength(request, maxLength) then
      ...
      file(tempFile, maxLength)(request).map(_.fold(result => Left(result), _ => Right(tempFile)))(using
        Execution.trampoline
      )
  }
  ...
  def default(maxLength: Option[Long]): BodyParser[AnyContent] = using { request =>
    if request.hasBody then anyContent(maxLength)
   ...
   *   If empty file uploads are allowed (no matter if filename or file is empty)
   */
  def multipartFormData(
      maxLength: Long,
      allowEmptyFiles: Boolean
  ): BodyParser[MultipartFormData[TemporaryFile]] =
    multipartFormData(
      Multipart.handleFilePartAsTemporaryFile(temporaryFileCreator),
      maxLength,
      allowEmptyFiles
  ...
  def multipartFormData[A](
    ...
    BodyParser("multipartFormData") { request =>
      val bodyAccumulator =
        Multipart
          .multipartParser(DefaultMaxTextLength, allowEmptyFiles, filePartHandler, errorHandler)
          .apply(request)
      enforceMaxLength(request, maxLength, bodyAccumulator)
    }
