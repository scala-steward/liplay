...
/**
 * An action is essentially a (Request[A] => Result) function that handles a request and generates a result to
...
trait BodyParser[+A] extends (RequestHeader => Accumulator[ByteString, Either[Result, A]]):
  ...
  /**
   * Uses the provided function to transform the BodyParser's computed result when the request body has been
   * parsed.
   *
   * @param f
   ...
   */
  def map[B](f: A => B)(using ec: ExecutionContext): BodyParser[B] =
   ...
   */
  def mapM[B](f: A => Future[B])(using ec: ExecutionContext): BodyParser[B] =
    // prepare execution context as body parser object may cross thread boundary
    implicit val pec = ExecCtxUtils.prepare(ec)
    new BodyParser[B]:
  ...
  /**
   * Uses the provided function to validate the BodyParser's computed result when the request body has been
   * parsed.
   *
   * The provided function can produce either a direct result, which will short circuit any further Action, or
   * a value of type B.
   *
   * Example:
   * {{{
   ...
   *   the transformed body parser
   */
  def validate[B](f: A => Either[Result, B])(using ec: ExecutionContext): BodyParser[B] =
    // prepare execution context as body parser object may cross thread boundary
    implicit val pec = ExecCtxUtils.prepare(ec)
    new BodyParser[B]:
   ...
   *   [[validate]]
   */
  def validateM[B](f: A => Future[Either[Result, B]])(using ec: ExecutionContext): BodyParser[B] =
    // prepare execution context as body parser object may cross thread boundary
    implicit val pec = ExecCtxUtils.prepare(ec)
    new BodyParser[B]:
      def apply(request: RequestHeader) =
...
/**
 * A builder for generic Actions that generalizes over the type of requests. An ActionFunction[R,P] may be
 * chained onto an existing ActionBuilder[R] to produce a new ActionBuilder[P] using andThen. The critical
 * (abstract) function is invokeBlock. Most users will want to use ActionBuilder instead.
 *
 * @tparam R
...
trait ActionFunction[-R[_], +P[_]]:
  ...
  /**
   * Invoke the block. This is the main method that an ActionBuilder has to implement, at this stage it can
   * wrap it in any other actions, modify the request object or potentially use a different class to represent
   * the request.
   *
   * @param request
   *   The request
...
trait ActionBuilder[+R[_], B] extends ActionFunction[Request, R]:
  ...
  /**
   * Constructs an [[ActionBuilder]] with the given [[BodyParser]]. The result can then be applied directly to
   * a block.
   *
   * For example:
   * {{{
  ...
  final def apply[A](bodyParser: BodyParser[A]): ActionBuilder[R, A] = new ActionBuilder[R, A]:
    ...
    protected override def composeParser[T](bodyParser: BodyParser[T]): BodyParser[T] =
      self.composeParser(bodyParser)
    protected override def composeAction[T](action: Action[T]): Action[T] = self.composeAction(action)
    override def invokeBlock[T](request: Request[T], block: R[T] => Future[Result]) =
  ...
  /**
   * Compose the parser. This allows the action builder to potentially intercept requests before they are
   * parsed.
   *
   * @param bodyParser
...
object ActionBuilder:
  class IgnoringBody()(using ec: ExecutionContext)
   ...
   */
  private[play] lazy val ignoringBody: ActionBuilder[Request, AnyContent] = new IgnoringBody()(using
 ...
 *
 * This trait is used for binding, since some dependency injection frameworks doesn't deal with types very
 * well.
 */
trait DefaultActionBuilder extends ActionBuilder[Request, AnyContent]

object DefaultActionBuilder:
  def apply(parser: BodyParser[AnyContent])(using ec: ExecutionContext): DefaultActionBuilder =
    new DefaultActionBuilderImpl(parser)

class ActionBuilderImpl[B](val parser: BodyParser[B])(using val executionContext: ExecutionContext)
...
/**
 * A simple kind of ActionFunction which, given a request (of type R), may either immediately produce a Result
 * (for example, an error), or call its Action block with a parameter (of type P). The critical (abstract)
 * function is refine.
 */
trait ActionRefiner[-R[_], +P[_]] extends ActionFunction[R, P]:

  /**
   * Determine how to process a request. This is the main method than an ActionRefiner has to implement. It
   * can decide to immediately intercept the request and return a Result (Left), or continue processing with a
   * new parameter of type P (Right).
   *
   * @param request
   *   the input request
   * @return
...
/**
 * A simple kind of ActionRefiner which, given a request (of type R), unconditionally transforms it to a new
 * parameter type (P) to be passed to its Action block. The critical (abstract) function is transform.
 */
trait ActionTransformer[-R[_], +P[_]] extends ActionRefiner[R, P]:

  /**
   * Augment or transform an existing request. This is the main method that an ActionTransformer has to
   * implement.
   *
   * @param request
   *   the input request
  ...
  final def refine[A](request: R[A]) = transform(request).map(Right(_))(using executionContext)

/**
 * A simple kind of ActionRefiner which, given a request (of type R), may either immediately produce a Result
 * (for example, an error), or continue its Action block with the same request. The critical (abstract)
 * function is filter.
 */
trait ActionFilter[R[_]] extends ActionRefiner[R, R]:

  /**
   * Determine whether to process a request. This is the main method that an ActionFilter has to implement. It
   * can decide to immediately intercept the request and return a Result (Some), or continue processing
   * (None).
   *
   * @param request
   *   the input request
  ...
  protected final def refine[A](request: R[A]) =
    filter(request).map(_.toLeft(request))(using executionContext)
