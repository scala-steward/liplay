...
object CoordinatedShutdownSupport:
  ...
  /**
   * Shuts down the provided `ActorSystem` synchronously, starting from the configured phase. This method
   * blocks until the actor system has fully shut down, or the duration exceeds timeouts for all coordinated
   * shutdown phases.
   *
   * @param actorSystem
   *   the actor system to shut down
  ...
  def syncShutdown(actorSystem: ActorSystem, reason: CoordinatedShutdown.Reason): Unit =
    ...
    logger.info(
      s"Starting synchronous coordinated shutdown with ${reason} reason and ${shutdownTimeout} timeout"
    )
